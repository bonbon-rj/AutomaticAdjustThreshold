...
int KmeansGetThreshold(cv::Mat &frame)
{
	...
	cv::Mat pixels(width * height, 1, CV_32F); //行 列
	for (int i = 0; i < height; i++)
	{
		uchar *row_ptr = frame.ptr<uchar>(i);
		for (int j = 0; j < width; j++)
		{
			float data = static_cast<float>(row_ptr[j]);

			// pixels.at<float>(i * width + j, 1) = data;
			float *pixel = pixels.ptr<float>(i * width + j);
			*pixel = data;
		}
	}
	...
	kmeans(pixels, clusterCount, labels, criteria, 3, cv::KMEANS_RANDOM_CENTERS);
	...
	{
		int *labeldata = labels.ptr<int>(i);
		float *pixeldata = pixels.ptr<float>(i);
...
void AdjustThreshold(void)
{
	...
	while (!end)
	{
		...
		std::cout << "\n";
		std::cout << "Estimate:";
		std::cout << "hmin:" << EstimatedHmin << ","
				  << "smin:" << EstimatedSmin << ","
				  << "vmin:" << EstimatedVmin << std::endl;
		...
		//打印输出
		std::cout << "Final:";
		std::cout << "hmin:" << HsvMin[0] << ","
				  << "smin:" << HsvMin[1] << ","
				  << "vmin:" << HsvMin[2] << std::endl;
		...
		//打印输出
		std::cout << "Final:";
		std::cout << "hmax:" << HsvMax[0] << ","
				  << "smax:" << HsvMax[1] << ","
				  << "vmax:" << HsvMax[2] << std::endl;
